...
vector<double> f(double time, vector<double> y) {
    return {y[1], 2 - 6 * time + 2 * pow(time, 3) + (time * time - 3) * exp(time) * sin(time) * (1 + cos(time)) +
                  cos(time) * (exp(time) + (time * time - 1) + pow(time, 4) - 3 * time * time) -
                  (time * time - 3) * y[1] - (time * time - 3) * cos(time) * y[0]};
}
...
double starting_condition(vector<double> c, vector<vector<double>> A, vector<double> b, double y_last) {
    double first = -10;
    double last = 10;
    auto result_a = solve(0, M_PI, 0.01, {0, first}, c, A, b, f);
    auto result_b = solve(0, M_PI, 0.01, {0, last}, c, A, b, f);
    while (abs(last - first) > 1e-6) {
        double median = (first + last) / 2;
        auto result_c = solve(0, M_PI, 0.01, {0, median}, c, A, b, f);
...
int main() {
    ...
    auto result = solve(0, M_PI, 0.01, {0, starting_condition(c, A, b, M_PI * M_PI)}, c, A, b, f);
